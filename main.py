...
const = [
    'int',
    'string_literals', 'float', 'char_literal', 'bool'
]
# important words
tokens = [
    'keyword', 'identifier', 'int',
    'operator', 'separator', 'string_literals', 'float', 'char_literal', 'datatype', 'bool', 'function-call'
]
# import file here
...
# find ints floats and char_literals within file
integers = re.findall('^[0-9]+$|[0-9]+', filecheck)
floatn = re.findall(r'[0-9]+\.[0-9]+', filecheck)
char_literals = re.findall(r'\'[^\'\\.]\'', filecheck)
...
bracket = ['(', ')', '{', '}', '[', ']']
datatype = ['int', 'float', 'char_literal', 'string_literals', 'bool', 'void']
punctuator = [',', ';', ':', ',']
arithop = ['-', '+', '*', '/', '%']
logop = ['||', '&&']
slogop = ['!']
...
class SymboLessThanable:
    ...
    def push(self, name, line_numbers, type, scope):
        new_node = symbolTableNode(name, line_numbers, type, scope)
...
class parser:
    ...
    def expression(self):
        ...
        const = [
            'int',
            'string_literals', 'float', 'char_literal'
        ]
    ...
    def data(self):
        ...
        data = self.lookahead.data['value']
        conslist = ['int', 'string_literals', 'float', 'char_literal', 'bool']
    ...
    def typeid(self):
        ...
        if (data == 'int'):
            ...
            self.match('float')
        elif (data == 'string_literals'):
            self.match('string_literals')
        elif (data == 'char_literal'):
